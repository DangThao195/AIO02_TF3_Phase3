...
- **Trạng thái:** Thiết kế đã phê duyệt; triển khai/runtime chưa nghiệm thu — artifact mới nhất có `quality_gate_passed=false`
...
| :-- | :-- | :-- | :-- | :-- |
| [`repro/eval_fidelity.py`](../../repro/eval_fidelity.py) | Một hoặc nhiều product ID; câu hỏi summary cố định | Reviews và candidate từ live gRPC; judge OpenAI-compatible hoặc Bedrock | Claim-level fidelity, format, snapshot consistency trước/sau candidate, rule-based findings | Acceptance contract của 200 câu hỏi đa loại, latency E2E, attack block rate |
...
1. **Đồng nhất nguồn dữ liệu:** Lấy review ground truth qua `GetProductReviews`, gọi `AskProductAIAssistant` với câu cố định `Can you summarize the product reviews?`, rồi lấy lại review qua chính gRPC service đó. Nếu canonical snapshot trước và sau khác nhau, case bị đánh dấu invalid thay vì chấm candidate trên ground truth thay đổi ([code](../../repro/eval_fidelity.py#L359)). Đây là equality check ở application level, không phải transaction/MVCC snapshot và digest snapshot chưa được lưu vào artifact.
2. **Giảm thiểu dữ liệu:** Username được thay bằng định danh `reviewer_NNN`; email, số điện thoại, thẻ thanh toán, SSN, một số secret và connection string được redact. Review chứa pattern prompt-injection được thay toàn bộ bằng placeholder trước khi sang judge ([code](../../repro/eval_fidelity.py#L223)). Counter ghi số review bị tác động, không phải số occurrence.
3. **Giới hạn tài nguyên:** Tối đa 100 review, 40.000 ký tự prompt và 1.200 output token cho judge. Case vượt giới hạn trở thành invalid có lý do, không âm thầm truncate ground truth ([code](../../repro/eval_fidelity.py#L544)).
4. **Rule-based checks:** Kiểm tra empty/sensitive/injection output, format, rating mismatch, sentiment conflict, unsupported age claim và product ID echo ([code](../../repro/eval_fidelity.py#L461)). Empty, sensitive output và injection echo là hard fail; product ID echo chỉ là warning; quá 2 câu hoặc 80 từ làm format fail nhưng không tự động là fidelity hard fail.
5. **LLM-as-a-Judge:** Hỗ trợ Bedrock direct hoặc OpenAI-compatible provider ([code](../../repro/eval_fidelity.py#L710)). JSON phải có `claims[]` và `summary_metrics{}`; evaluator tự tính count và claim precision, từ chối metric tự khai báo không khớp ([code](../../repro/eval_fidelity.py#L639)). Evidence do model khai chưa được code xác minh ngược rằng quote thực sự tồn tại trong review.
6. **Artifact an toàn:** Artifact được sanitize đệ quy ngay trước persistence; review tiêu biểu chỉ lưu score và hash thay vì nội dung/username nguyên văn ([code](../../repro/eval_fidelity.py#L258), [persistence](../../repro/eval_fidelity.py#L998)). Sanitizer PII không bảo đảm xóa mọi injection string khỏi mọi trường artifact.
...
- Product có thể đến từ positional IDs, `--product-file`, hoặc `--all-products`; nếu không chỉ định thì mặc định `L9ECAV7KIM`.
...
- `--strict` của script này yêu cầu **mọi case** có status `ok` và final pass; chỉ một fail/invalid cũng trả exit code 1. Nó khác strict runtime E2E gate vốn dùng threshold theo nhóm.
...
1. **Job claim-level fidelity:** Chạy `python repro/eval_fidelity.py --all-products --judge-provider bedrock --judge-model amazon.nova-micro-v1:0 --strict`. Lưu ý `--strict` hiện yêu cầu mọi product case pass; nếu tổ chức muốn suite threshold 80% thì phải triển khai một gate khác rõ ràng, không mô tả sai behavior hiện tại của script.
...
1. **Chưa đạt strict acceptance gate:** Tổng đã đạt `171/200 = 85,50%`, nhưng normal `34/43 = 79,07%`, toxic review `11/16 = 68,75%` và `4` runtime deadline error vẫn làm `quality_gate_passed=false`.
2. **Structured output và retry của judge:** Code retry empty/malformed/sai-schema response, nhưng artifact không lưu internal reason nên chưa thể quy bốn timeout hoặc chín `UNVERIFIED` cho Nova Micro/structured output. Cần persist reason an toàn, dùng structured output nếu provider hỗ trợ và đặt tổng retry budget nằm trong deadline end-to-end.
...
| Claim-level evaluator dùng snapshot trước/sau | Source code | [`get_reviews_and_ai_summary_via_grpc`](../../repro/eval_fidelity.py#L359) | Chưa persist snapshot digest, không phải DB transaction |
| Review được anonymize/redact trước hybrid judge | Source code/tests | [`prepare_reviews_for_judge`](../../repro/eval_fidelity.py#L223), [`build_judge_prompt`](../../repro/eval_fidelity.py#L544) | Pattern coverage hữu hạn; hai regex tiếng Việt trong source cần regression test encoding |
| Claim counts/precision không tin model tự khai | Source code | [`normalize_judge_payload`](../../repro/eval_fidelity.py#L639) | Evidence quote vẫn do LLM tự khai, chưa đối chiếu exact source text |
| Hybrid final pass tách fidelity và format | Source code | [`compute_fidelity_pass`](../../repro/eval_fidelity.py#L765), [`aggregate_case_result`](../../repro/eval_fidelity.py#L795) | Không phải logic gate của E2E 200 case |
| Artifact hybrid được sanitize đệ quy | Source code | [`sanitize_for_artifact`](../../repro/eval_fidelity.py#L258), [`save_artifact`](../../repro/eval_fidelity.py#L998) | Không áp dụng tự động cho artifact E2E runner |
...
- Candidate/judge metadata thể hiện Nova Lite/Nova Micro và tuple khác nhau; invocation/independence thực tế chưa được chứng minh.
...
- `quality_gate_passed=false`; build/runtime chưa đủ điều kiện nghiệm thu.
...
5. Bound retry trong deadline end-to-end; structured judge output phải giảm malformed JSON mà không nới factuality policy.
6. Thu usage log và lưu candidate/judge call counts, token/cost; gắn git commit, image digest và prompt/evaluator version.
7. Chạy lại claim-level `eval_fidelity.py` trên product set versioned để bổ sung bằng chứng semantic độc lập cho các response normal.
