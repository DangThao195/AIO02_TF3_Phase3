...
## 📸 10. Bộ 3 Snapshots Dán Jira Ticket (Trích Xuất Từ Tệp Bằng Chứng Thực Tế trong Repo)

> [!IMPORTANT]
> **Cam Kết Dữ Liệu Thật 100% (Real Runtime Evidence):** Tất cả bản ghi trace bên dưới được trích xuất trực tiếp từ tệp bằng chứng chạy thực tế [repro/artifacts/llm_trace_smoketest_20260727T000000Z.json](../../repro/artifacts/llm_trace_smoketest_20260727T000000Z.json) trên container `aie1-product-reviews:trace-current` kết nối Amazon Bedrock API thật.

### 📸 Snapshot 1: Trace Request Thật Chạy Lần Đầu - Cold Run (Bedrock Nova Lite + Nova Micro Call Chain)
> **Mô tả:** Trích xuất từ tệp trace thực tế `trace_id = "6430920b-c1a2-4e38-b77d-3f8a2d019c55"` (câu hỏi: *"Do reviewers say the kit removes dust and fingerprints without leaving residue?"*, product_id: `"L9ECAV7KIM"`).
...
### 📸 Snapshot 2: Trace Request Thật Chạy Lần Hai - Cache Hit Run (Bypass LLM Call)
> **Mô tả:** Trích xuất từ tệp trace thực tế `trace_id = "4e4cd351-82b3-4f19-a6c0-9d7e5f102a48"`. Minh chứng khi Cache Hit (`outcome: "cache_hit"`), độ trễ giảm xuống còn **18.3 ms** và trace trỏ ngược về `source_trace_id = "6430920b-c1a2-4e38-b77d-3f8a2d019c55"`.
...
> **Mô tả:** Trích xuất từ tệp bằng chứng đo lường thực tế [repro/artifacts/cost_latency_comparison.json](../../repro/artifacts/cost_latency_comparison.json) & [cost_latency_baseline.json](../../repro/artifacts/cost_latency_baseline.json).
