...
| **Chi phí Hạ tầng & Tài nguyên (Cost & Resources)** | **Rất Thấp (Tối ưu)**<br>- **Local**: $0 (Tận dụng Postgres sẵn có).<br>- **AWS**: Không phát sinh thêm chi phí phần cứng (chỉ sử dụng chung instance RDS/Aurora hiện tại).<br>- **Lưu trữ**: Lưu trên ổ đĩa SSD (EBS GP3 có giá rất rẻ, chỉ khoảng **~$0.08 / GB / tháng**). | **Cao hơn (Có thể tối ưu qua K8s)**<br>- **Local**: Tốn thêm RAM để chạy container Redis riêng biệt (~50-100MB RAM).<br>- **AWS (ElastiCache)**: Tốn thêm chi phí tạo cluster Redis. Một instance `cache.t4g.medium` (3GB RAM) có giá khoảng **~$30 / tháng**. Nếu chạy dạng High-Availability (Primary-Replica) sẽ là **~$60 / tháng**.<br>- **Giải pháp tối ưu (K8s Infra Cache)**: Để giảm chi phí về **$0**, hệ thống có thể triển khai **Redis/Valkey tự host (Self-hosted) trực tiếp trong Kubernetes Cluster** (dùng Bitnami Helm Chart). Điều này tận dụng tài nguyên RAM dư thừa của worker nodes hiện tại mà không phát sinh thêm hóa đơn dịch vụ AWS mới.<br>- **Lưu trữ**: RAM có chi phí đắt gấp **~50 - 90 lần** so với SSD per GB (khoảng **~$5 - $7 / GB / tháng**). | **PostgreSQL Thắng hoàn toàn về mặt chi phí**:<br>- Tiết kiệm đáng kể ngân sách ban đầu và tài nguyên phần cứng, đặc biệt phù hợp khi kích thước tập cache chưa quá lớn.<br>- Tuy nhiên, nếu tối ưu bằng phương án **K8s In-cluster Cache**, Redis/Valkey vẫn có thể giảm chi phí hạ tầng bổ sung về **$0**. |
...
| **Deploy & Vận hành trên AWS Cloud** | **Amazon RDS / Aurora Serverless**<br>- Tái sử dụng Instance RDS hiện tại, chỉ tăng nhẹ tải đọc/ghi. Vận hành tập trung trên một DB duy nhất. | **Amazon ElastiCache / MemoryDB**<br>- Cần quản lý thêm một dịch vụ chuyên biệt (ElastiCache). Có cơ chế Cluster / Replication tự động phân mảnh và sao lưu.<br>- **Giải pháp tối ưu hóa chi phí (K8s Shared Service)**: Triển khai Valkey/Redis dưới dạng một dịch vụ dùng chung chạy ngay trên K8s cluster (In-cluster Shared Service). Mọi Task Force (AIE1, AIE2, v.v.) kết nối chung vào đây để tiết kiệm 100% chi phí quản lý AWS ElastiCache.<br>- Giảm tải hoàn toàn truy vấn đọc/ghi cache cho PostgreSQL chính để dành tài nguyên cho các nghiệp vụ ACID quan trọng khác. | **Redis Thắng về mặt kiến trúc**:<br>- Giúp tách biệt rõ ràng lớp lưu trữ chính (Database) và lớp bộ đệm (Caching), tăng độ bền vững và khả năng scale cho hệ thống lớn. |
...
> Toàn bộ các kho lưu trữ tự host trong cluster đã được yêu cầu gỡ bỏ hoàn toàn để chuyển sang các dịch vụ Managed Services chính thức. Mặc dù phương án **K8s In-cluster Cache (Valkey/Redis tự host)** là giải pháp tối ưu chi phí hạ tầng về **$0**, nhưng để đáp ứng quy định tuân thủ bảo mật và vận hành quy mô lớn của doanh nghiệp, hệ thống bắt buộc dịch chuyển sang **AWS ElastiCache**. Việc quản lý tài nguyên và tối ưu hóa hóa đơn AWS ElastiCache lúc này được chuyển giao cho Platform Team chịu trách nhiệm điều phối.
