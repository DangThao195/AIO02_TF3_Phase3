...
### 2.3. Cơ Chế Invalidation Dựa Trên Phiên Bản & Mô Hình (Version & Model-Based Invalidation)
Thay vì thực hiện các lệnh xóa cache thủ công (`DELETE`) vốn tốn tài nguyên và dễ gây lỗi khi có review mới hoặc khi thay đổi mô hình LLM, hệ thống áp dụng cơ chế khóa cache động dựa trên phiên bản dữ liệu và mô hình:
...
- Để tránh xung đột khóa và ô nhiễm cache khi nâng cấp hoặc thay đổi mô hình LLM trong tương lai (ví dụ: chuyển từ Nova Lite sang Nova Micro hoặc GPT-4o), mã nhận diện mô hình (`model_id`) bắt buộc phải tham gia vào công thức tạo khóa.
...
- Khi có review mới hoặc khi hệ thống chuyển đổi mô hình LLM $\rightarrow$ Khóa Cache tự động đổi $\rightarrow$ Tự động gây ra **Cache Miss** và kích hoạt việc sinh dữ liệu mới với mô hình mới. Các cache cũ của phiên bản trước hoặc của mô hình cũ sẽ tự hết hạn dựa trên cơ chế TTL mà không cần xóa thủ công.
...
flowchart TD
    ...
    InputGuard -->|An toàn| KeyGen["Tạo Cache Key dựa trên review_version & model_id"]
...
def generate_cache_key(product_id: str, review_version: str, model_id: str, question: str) -> str:
    ...
    # Bổ sung model_id vào raw_key để tránh xung đột cache khi đổi LLM
