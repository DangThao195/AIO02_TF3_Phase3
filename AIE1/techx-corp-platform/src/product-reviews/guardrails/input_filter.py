...
ATTACK_PATTERNS: List[Tuple[re.Pattern, str]] = [
    ...
    (re.compile(r"(bo\s+qua|quen|ghi\s+de).{0,80}(huong\s+dan|chi\s+dan|quy\s+tac|noi\s+bo|he\s+thong)", re.IGNORECASE),
     "SYSTEM_OVERRIDE"),
    (re.compile(r"\bno\s+content\s+polic(?:y|ies)\b|\bwithout\s+content\s+(?:polic(?:y|ies)|restrictions?|limits?)\b", re.IGNORECASE),
     "JAILBREAK"),
    (re.compile(r"(act|pretend|behave|respond|roleplay|playing\s+the\s+role).{0,120}\bno\s+(?:restrictions?|limits?)\b", re.IGNORECASE),
     "JAILBREAK"),
