...
param(
  ...
  [string]$CandidateModel = $(if ($env:LLM_MODEL) { $env:LLM_MODEL } else { "amazon.nova-lite-v1:0" }),
  [string]$JudgeModel = $(if ($env:JUDGE_MODEL) { $env:JUDGE_MODEL } else { "amazon.nova-micro-v1:0" }),
  [string]$UsageLog = "",
  [string]$PricingJson = "",
