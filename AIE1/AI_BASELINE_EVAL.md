...
_Đề xuất các giải pháp kỹ thuật nâng cấp tầng AI trong các tuần tiếp theo, được phân loại theo các trụ cột kiến trúc._

### 1. Phân nhóm Backlog Kỹ Thuật

| STT | Trụ cột / Giải pháp Kỹ thuật | Lý do / Lợi ích | Rủi ro (1-5) | Tác động Business | Tài liệu thiết kế | Trạng thái |
| :--- | :--- | :--- | :---: | :---: | :--- | :--- |
| **A** | **TẦNG KẾT NỐI & MULTI-PROVIDER** | | | | | |
| **1** | Tích hợp trực tiếp SDK `boto3` (AWS Bedrock) | Sửa `product_reviews_server.py` để sử dụng trực tiếp SDK `boto3` (Converse API) của AWS thay thế cho thư viện `OpenAI`. Loại bỏ được LiteLLM Proxy trên EKS, giảm 1 hop mạng trung gian, tối ưu hóa độ trễ (Latency). | `2` | **High** (Tối ưu SLA) | [BEDROCK_INTEGRATION_PROPOSAL.md](docs/analysis/BEDROCK_INTEGRATION_PROPOSAL.md) | Sẵn sàng / Chờ code |
| **B** | **ĐỘ TIN CẬY & CHỊU LỖI (RELIABILITY)** | | | | | |
| **2** | Tích hợp cơ chế Thử lại & Trễ lũy thừa (Retry & Exponential Backoff) | Tự động thử lại 3 lần kèm ngẫu nhiên độ trễ (Full Jitter) khi gặp lỗi mạng hoặc lỗi tạm thời (HTTP 429, 500, 503) từ Bedrock trước khi kích hoạt Fallback. | `2` | **High** (Tính chống chịu lỗi) | [LLM_RETRY_BACKOFF.md](docs/analysis/LLM_RETRY_BACKOFF.md) | Sẵn sàng / Chờ code |
| **3** | Triển khai Graceful Fallback 3 tầng | Bọc khối lệnh `try-except` xung quanh LLM call để tự động chuyển hướng: LLM thật (T1) → PostgreSQL Cache (T2) → Static Message (T3) khi Bedrock bị lỗi kéo dài. | `1` | **High** (Tránh treo storefront) | [0002-fallback-mechanism.md](docs/adr/0002-fallback-mechanism.md) | Đang thiết kế |
| **C** | **HIỆU NĂNG & TỐI ƯU CHI PHÍ** | | | | | |
| **4** | Cơ chế Caching phản hồi LLM | Sử dụng băm khóa (Hash Keys) dựa trên `product_id` (tóm tắt chung) hoặc `SHA256(product_id + normalize(question))` (hỏi đáp Q&A) để lưu cache trong DB/Redis, giảm 80% chi phí token và giảm latency xuống < 10ms. | `2` | **High** (Giảm 80% chi phí AI) | [LLM_CACHING_DESIGN.md](docs/analysis/LLM_CACHING_DESIGN.md) | Sẵn sàng / Chờ code |
| **5** | Phản hồi dạng luồng (Streaming Response) | Chuyển đổi sang gRPC Server-side Streaming kết hợp API `converse_stream` của Bedrock để trả về text dạng chunk-by-chunk, tối ưu hóa TTFT xuống dưới 200ms. | `3` | **Medium** (Tăng trải nghiệm UX) | [LLM_STREAMING.md](docs/analysis/LLM_STREAMING.md) | Sẵn sàng / Chờ code |
| **D** | **BẢO MẬT & LỌC DỮ LIỆU (SECURITY)** | | | | | |
| **6** | Middleware lọc thông tin nhạy cảm PII | Viết hàm regex tự động che giấu Email, SĐT trong review gốc thành `[EMAIL]`, `[PHONE_NUMBER]` trước khi gửi payload lên Bedrock. | `1` | **Medium** (Bảo mật dữ liệu) | Mục 4 trong [AI_BASELINE_EVAL.md](AI_BASELINE_EVAL.md#L650) | Đang thiết kế |
| **7** | Chặn Prompt Injection & Jailbreak | Bổ sung input validation lọc các từ khóa độc hại từ người dùng cố tình thay đổi system prompt hoặc can thiệp thực thi tools bậy bạ. | `2` | **High** (Bảo mật tầng AI) | Mục 4 trong [AI_BASELINE_EVAL.md](AI_BASELINE_EVAL.md#L650) | Đang thiết kế |
| **E** | **MỞ RỘNG & TRẢI NGHIỆM CHAT** | | | | | |
| **8** | Đăng ký công cụ động (Dynamic Tool Registry) | Sử dụng class `ToolRegistry` và decorator `@register_tool` để đăng ký động và tự động điều phối (Dynamic Dispatch) các AI tools, loại bỏ code rẽ nhánh `if/elif` cồng kềnh. | `2` | **Medium** (Dễ bảo trì mã nguồn) | [LLM_DYNAMIC_TOOL_REGISTRY.md](docs/analysis/LLM_DYNAMIC_TOOL_REGISTRY.md) | Sẵn sàng / Chờ code |
| **9** | Bộ nhớ hội thoại đa lượt (Conversation Memory) | Lưu trữ ngữ cảnh tin nhắn theo phiên trò chuyện (`session_id`) vào Redis phân tán (được cài đặt tự động hết hạn TTL sau 30 phút). | `3` | **Medium** (Trò chuyện đa lượt) | [LLM_CONVERSATION_MEMORY.md](docs/analysis/LLM_CONVERSATION_MEMORY.md) | Sẵn sàng / Chờ code |
| **F** | **ĐÁNH GIÁ CHẤT LƯỢNG & SỬA LỖI LOGIC** | | | | | |
| **10** | Sửa lỗi rò rỉ mã sản phẩm (Product ID Leak) | Thay thế việc truyền trực tiếp mã sản phẩm trong nội dung prompt bằng `"this product"` ở cả prompt đầu vào và câu tổng hợp đầu ra để tránh LLM echo mã ID. | `1` | **High** (Quyền riêng tư) | - | Đang xử lý |
| **11** | Chuẩn hóa kiểu dữ liệu đầu ra của Tool | Đảm bảo `fetch_product_reviews` trả về kiểu `string` thay vì raw data trước khi append vào messages để tránh lỗi BadRequest (400) từ API LLM. | `1` | **High** (Tránh crash runtime) | - | Cần xử lý ngay |
| **12** | Tối ưu hóa hệ thống đánh giá (Fix 4 Eval Bottlenecks) | Khắc phục các điểm nghẽn của bộ eval bao gồm bất nhất Ground Truth, nhạy cảm dải số rating, và tích hợp eval trực tiếp vào runtime fallback. | `3` | **High** (Độ chính xác đánh giá) | [evaluation_bottlenecks.md](docs/analysis/evaluation_bottlenecks.md) | Backlog |
