...
from config import PROMETHEUS_URL, S3_BUCKET_NAME, MODEL_FEATURE_COLUMNS
...
class AnomalyDetector:
    ...
    def extract_features_realtime(self, service: str) -> pd.DataFrame:
        ...
        # Tính toán features — DÙNG CHUNG duy nhất hàm feature_engineering() của
        # train_anomaly_model_local.py (import trễ để tránh circular import ở module-load
        # time). SỬA LỖI: bản trước có 1 bản sao RIÊNG của công thức feature ở đây, bị lệch
        # (train/serve skew) so với script training — cụ thể `latency_deviation` dùng floor
        # cố định 1e-5 khiến tỷ lệ nổ số vô nghĩa khi rolling_median_1h ~ 0 (rất phổ biến ở
        # service traffic thưa). Từ nay CHỈ CÓ MỘT nguồn tính feature dùng chung cho cả
        # training, benchmark, và runtime production.
    ...
    def check_service_anomaly(self, service: str) -> dict:
        ...
        # SỬA LỖI: dùng chung config.MODEL_FEATURE_COLUMNS thay vì list lặp lại cục bộ,
        # tránh lệch feature set giữa các nơi trong codebase.
