...
from config import MODEL_FEATURE_COLUMNS, IFOREST_HYPERPARAMS, BENCHMARK_RANDOM_SEED
...
def generate_synthetic_data(service: str, duration_days: int = 14, is_anomaly_set: bool = False, anomaly_type: str = None) -> pd.DataFrame:
    ...
    Được seed bằng BENCHMARK_RANDOM_SEED (nhân với hash service+scenario) để
    A/B benchmark có thể tái lập kết quả giữa các lần chạy, đồng thời không
    sinh ra cùng một chuỗi số y hệt cho mọi (service, scenario) khác nhau.
    """
    seed_material = f"{service}|{duration_days}|{is_anomaly_set}|{anomaly_type}"
    seed_digest = int(hashlib.md5(seed_material.encode("utf-8")).hexdigest()[:8], 16)
    local_seed = (BENCHMARK_RANDOM_SEED + seed_digest) % (2**32 - 1)
...
def feature_engineering(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # rolling median 1h = 12 mẫu
    df["rolling_median_1h"] = df["latency_p90"].rolling(window=12, min_periods=1).median()

    # SỬA LỖI: guard chống nổ số vô nghĩa khi rolling_median_1h ~ 0 (rất phổ biến ở service
    # có traffic thưa như product-catalog/checkout, nơi latency_p90 thật gần như luôn = 0).
    # Bản gốc dùng floor cố định 1e-5 -> latency_deviation có thể nổ lên hàng trăm nghìn
    # (vd 861,663 khi rolling_median=~0) ngay TRONG CHÍNH dữ liệu train thật (do các đợt trễ
    # tự nhiên hiếm gặp), khiến model "quen" với việc tỷ lệ này cực đoan và mất khả năng phân
    # biệt khi gặp incident thật. Floor mới scale theo p95 latency_p90 của chính chuỗi đang xét
    # (tối thiểu 1.0 đơn vị) + clip ratio về khoảng hữu hạn [0, 50] — vẫn đủ lớn để phản ánh
    # bất thường thật (>=50x median vẫn là tín hiệu cực mạnh) nhưng không còn nổ số vô nghĩa.
    _lat_p95 = df["latency_p90"].quantile(0.95)
    _floor = max(float(_lat_p95) * 0.05, 1.0)
    df["latency_deviation"] = (df["latency_p90"] / (df["rolling_median_1h"] + _floor)).clip(upper=50)
    ...
    # SỬA LỖI: bản gốc dùng ngưỡng tuyệt đối cứng "rps > 100" LÀM ĐIỀU KIỆN BẮT BUỘC.
    # Với 5/7 service thật (checkout, payment, product-reviews, recommendation, shipping),
    # RPS thật KHÔNG BAO GIỜ chạm 100 -> feature này LUÔN = 0 cho các service đó, khiến
    # model không có tín hiệu "đây là giờ cao điểm của CHÍNH service này" -> traffic cao
    # điểm thật bị báo động nhầm (đã verify: 61/61 FP của recommendation đều rơi vào đúng
    # top-5% RPS thật). Floor giờ tính TƯƠNG ĐỐI theo median RPS của chính chuỗi đang xét
    # (thay vì số tuyệt đối áp đặt), để mọi service — bất kể quy mô traffic — đều có tín
    # hiệu "cao điểm" đúng nghĩa tương đối với chính nó.
    df["rolling_median_rps_1h"] = df["rps"].rolling(window=12, min_periods=1).median()
    _series_median_rps = max(df["rps"].median(), 1e-3)
    _relative_floor = 2.0 * _series_median_rps
    df["is_high_traffic_period"] = (
        (df["rps"] > _relative_floor) & (df["rps"] > 1.5 * df["rolling_median_rps_1h"])
    ).astype(int)
...
def train_and_evaluate():
    ...
    # LƯU Ý: lấy từ config.MODEL_FEATURE_COLUMNS — nguồn DUY NHẤT dùng chung
    # cho cả Old Engine (AnomalyDetector) và Enhanced Engine (EnhancedAnomalyDetector)
    # để đảm bảo A/B benchmark so sánh công bằng, không lệch feature set.
    ...
    for service in SERVICES:
        ...
        # 3. Huấn luyện mô hình Isolation Forest (hyperparams dùng chung config.IFOREST_HYPERPARAMS)
