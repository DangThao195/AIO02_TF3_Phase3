...
region = "us-east-1"
collection_endpoint = "trr490g18kpnofbpupe3.us-east-1.aoss.amazonaws.com"
...
def run():
    ...
    print("Connecting to OpenSearch Serverless data plane...")
    client = OpenSearch(
    ...
    try:
        ...
        print(f"-> Index did not exist or delete failed (continuing): {e}")
        
    time.sleep(2)
    
    # Định nghĩa cấu hình index tối ưu cho Bedrock (Sử dụng engine faiss, l2 space)
    index_body = {
        "settings": {
            "index.knn": True
        },
    ...
    print(f"Creating new Vector Index with FAISS engine...")
    try:
        res = client.indices.create(index=index_name, body=index_body)
        print(f"-> SUCCESS: Index recreated: {res}")
    except Exception as e:
        print(f"-> FAILED: Recreating index failed: {e}")
        return
        
    time.sleep(5)
    
    # -------------------------------------------------------------
    # BƯỚC 5: Tạo Bedrock Knowledge Base & Data Source
    # -------------------------------------------------------------
