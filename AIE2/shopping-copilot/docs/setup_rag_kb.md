...
1. Truy cập **Amazon Bedrock** Console $\rightarrow$ chọn **Knowledge bases** $\rightarrow$ chọn **Create knowledge base**.
...
2. Gọi API để Bedrock KB cập nhật các thay đổi trên S3 (Chỉ cập nhật file mới/thay đổi):
   ```python
   import boto3
   client = boto3.client('bedrock-agent', region_name='ap-southeast-1')
