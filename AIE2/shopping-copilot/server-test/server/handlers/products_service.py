...
class ProductsService(grpc_svc.ProductsServicer):
    ...
    async def GetProduct(self, request, context):
        row = await db.fetchrow(
            "SELECT id, name, description, picture, price_currency_code, price_units, price_nanos, categories FROM catalog.products WHERE id = $1",
            request.id,
        )
        if row is None:
    ...
    async def ListProducts(self, request, context):
        rows = await db.fetch(
            "SELECT id, name, description, picture, price_currency_code, price_units, price_nanos, categories FROM catalog.products ORDER BY name",
        )
