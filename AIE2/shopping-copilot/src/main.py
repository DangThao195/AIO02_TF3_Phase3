...
def api_cache_metrics():
    """
    Trả về cache metrics để test runner có thể đánh giá hiệu năng.

    Returns:
        {
            "summary": {
                "genai_hits_exact": int,
                "genai_hits_semantic": int,
                "genai_hits_global": int,
                "genai_misses": int,
                "genai_hit_rate_pct": float,
                "backend": str,
    ...
    total = sum(cleared_items.values())

    return {
        "status": "ok",
        "message": f"Cleared {total} total entries (cart data preserved)",
        "details": cleared_items,
        "note": "Cart data was NOT cleared",
    }
...
def api_cache_metrics():
    ...
    Returns hit-rate, latency breakdown, token savings, cost analysis,
    and per-tier statistics (exact / semantic / global).
    Dùng để đo lường lợi ích cache bằng số thật.
    """
    from src.memory.genai_cache import get_genai_cache_store

    genai_cache = get_genai_cache_store()
    cache_stats = genai_cache.stats()

    # Tool-level cache stats
    agent = _get_agent()
    tool_cache_stats = agent.cache_store.stats() if agent else {}

    return {
        "genai_cache": cache_stats,
        "tool_cache": tool_cache_stats,
        "summary": {
            "genai_hit_rate_pct": cache_stats.get("hit_rate_pct", 0),
            "genai_total_requests": cache_stats.get("total_requests", 0),
            "genai_total_hits": cache_stats.get("total_hits", 0),
            "genai_hits_exact": cache_stats.get("hits_exact", 0),
            "genai_hits_semantic": cache_stats.get("hits_semantic", 0),
            "genai_hits_global": cache_stats.get("hits_global", 0),
            "genai_misses": cache_stats.get("misses", 0),
            "tool_hit_rate_pct": tool_cache_stats.get("hit_rate_pct", 0),
            "cache_ttl_seconds": cache_stats.get("ttl_seconds", 0),
